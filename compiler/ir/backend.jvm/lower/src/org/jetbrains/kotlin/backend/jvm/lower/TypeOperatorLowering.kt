                ...
                with(builder) {
                    irLetS(argument, irType = context.irBuiltIns.anyNType) { tmp ->
                        if (backendContext.state.unifiedNullChecks) {
                            ...
                            // We have to generate a null check here, because even if argument is of non-null type, it can be uninitialized
                            // value, which is 'null' for reference types in JMM.
                            ...
                            irBlock {
                                +irCall(backendContext.ir.symbols.checkNotNullWithMessage).apply {
                                    ...
                                    putValueArgument(1, irString("null cannot be cast to non-null type ${type.render()}"))
                                }
